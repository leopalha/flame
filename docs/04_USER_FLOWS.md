# 🔄 FLAME - USER FLOWS

**Versão:** 3.5.0
**Última Atualização:** 07/12/2024
**Sincronizado com:** Código-fonte e PRD v3.5.0 (Sprint A - Pagamento com Atendente + Troco)

## ÍNDICE

1. [Fluxos do Cliente](#1-fluxos-do-cliente)
2. [Fluxos do Staff](#2-fluxos-do-staff)
3. [Fluxos Administrativos](#3-fluxos-administrativos)
4. [Fluxos do Sistema](#4-fluxos-do-sistema)
5. [Mapeamento Técnico](#5-mapeamento-técnico)
6. [Divergências Conhecidas](#6-divergências-conhecidas)

---

## LEGENDA DE STATUS

| Símbolo | Significado |
|---------|-------------|
| ✅ | Implementado e funcionando |
| ⚠️ | Parcialmente implementado ou com divergência |
| ❌ | Não implementado |
| 🔄 | Em desenvolvimento |

---

## 1. FLUXOS DO CLIENTE

### 1.1 Cadastro e Autenticação

#### 1.1.1 Primeiro Acesso (via QR da Mesa)

```
┌─────────────────────────────────────────────────────────┐
│                    ENTRADA VIA QR                       │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  [Cliente escaneia QR na mesa]                         │
│              │                                          │
│              ▼                                          │
│  ┌─────────────────────┐                               │
│  │ URL: /qr/mesa-{id}  │                               │
│  └──────────┬──────────┘                               │
│              │                                          │
│              ▼                                          │
│  ┌─────────────────────┐                               │
│  │ Sistema detecta:    │                               │
│  │ • Mesa ID           │                               │
│  │ • Salva em session  │                               │
│  └──────────┬──────────┘                               │
│              │                                          │
│              ▼                                          │
│      ┌───────────────┐                                 │
│      │ Tem sessão?   │                                 │
│      └───────┬───────┘                                 │
│              │                                          │
│      ┌───────┴───────┐                                 │
│      │               │                                 │
│      ▼               ▼                                 │
│    [SIM]           [NÃO]                               │
│      │               │                                 │
│      ▼               ▼                                 │
│  /cardapio      /bem-vindo                             │
│  (mesa já       ┌─────────────┐                        │
│   definida)     │ Opções:     │                        │
│                 │ • Entrar    │                        │
│                 │ • Cadastrar │                        │
│                 │ • Só ver    │                        │
│                 │   cardápio  │                        │
│                 └─────────────┘                        │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

#### 1.1.2 Cadastro Completo (Tradicional) ⚠️ ATUALIZADO

```
┌─────────────────────────────────────────────────────────────────────┐
│                 CADASTRO COMPLETO (/register)                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  Formulário:                                                         │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Nome Completo *:   [_______________________]                │    │
│  │ Email *:           [_______________________]                │    │
│  │ Celular *:                                                  │    │
│  │ ┌─────────────────────────────────────────────────────────┐ │    │
│  │ │ [🇧🇷 Brasil +55 ▼] [(21) 99999-9999  ]                 │ │    │
│  │ └─────────────────────────────────────────────────────────┘ │    │
│  │ ↑ Ao clicar, abre lista pesquisável de países:              │    │
│  │   🔍 Buscar país...                                         │    │
│  │   🇧🇷 Brasil +55 | 🇺🇸 EUA +1 | 🇵🇹 Portugal +351 | ...    │    │
│  │                                                             │    │
│  │ ⚡ Sistema detecta automaticamente pelo país do telefone:   │    │
│  │    - Se Brasil → mostra campo CPF                           │    │
│  │    - Se outro país → mostra campo ID Estrangeiro            │    │
│  │                                                             │    │
│  │ Data de Nascimento *: [DD/MM/AAAA]                          │    │
│  │ ⚠️ Você precisa ter 18 anos ou mais                         │    │
│  │                                                             │    │
│  │ [Se país = Brasil - detectado automaticamente]              │    │
│  │ CPF *:             [000.000.000-00      ] ✓ Validado        │    │
│  │                                                             │    │
│  │ [Se país ≠ Brasil - detectado automaticamente]              │    │
│  │ Nº Identificação *: [Passaporte/RNE     ]                   │    │
│  │                                                             │    │
│  │ Senha:            [___________] (min 6 chars)               │    │
│  │ Confirmar Senha:  [___________]                             │    │
│  │                                                             │    │
│  │ [✓] Aceito os Termos de Uso e Política de Privacidade      │    │
│  │ [✓] Declaro ter 18 anos ou mais                            │    │
│  │                                                             │    │
│  │ [          Criar Conta          ]                           │    │
│  │                                                             │    │
│  │ ─────────────────── ou ───────────────────                  │    │
│  │                                                             │    │
│  │ [   G   Cadastrar com Google    ]                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                │                                     │
│                     POST /api/auth/register                         │
│         { nome, email, celular, birthDate, cpf/foreignId, password }│
│                                │                                     │
│                                ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Backend:                                                    │    │
│  │ 1. Valida unicidade: email, celular (e cpf se fornecido)   │    │
│  │ 2. Cria User com profileComplete: true, phoneVerified: false│   │
│  │ 3. Gera código SMS 6 dígitos (expira em 5 min)             │    │
│  │ 4. Envia SMS via Twilio                                    │    │
│  │ 5. Retorna { userId, celular, smsExpiry }                  │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                │                                     │
│                                ▼                                     │
│  Tela de Verificação SMS:                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │        Código Enviado!                                      │    │
│  │    Enviamos um código para +55 21 99999-9999               │    │
│  │                                                             │    │
│  │    [ 0 ] [ 0 ] [ 0 ] [ 0 ] [ 0 ] [ 0 ]                     │    │
│  │                                                             │    │
│  │    [     Verificar Código      ]                            │    │
│  │    Não recebeu? [Reenviar]                                  │    │
│  │    [     Voltar     ]                                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                │                                     │
│                     POST /api/auth/verify-sms                       │
│                    { celular, code }                                │
│                                │                                     │
│                  ┌─────────────┴─────────────┐                      │
│                  │                           │                      │
│              Código OK               Código Errado                  │
│                  │                           │                      │
│                  ▼                           ▼                      │
│  ┌────────────────────┐       ┌────────────────────────────┐       │
│  │ phoneVerified=true │       │ smsAttempts++ (máx 3)      │       │
│  │ smsCode=null       │       │ Erro: "Código incorreto"   │       │
│  │ Gera JWT token     │       │ Tentativas restantes: X    │       │
│  │ Envia SMS welcome  │       └────────────────────────────┘       │
│  └────────────────────┘                                             │
│           │                                                          │
│           ▼                                                          │
│  Redirect: / (home baseado na role)                                 │
│  Toast: "Cadastro concluído! Bem-vindo ao FLAME!"                   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

**Mapeamento Técnico:**

| Componente | Arquivo | Função |
|------------|---------|--------|
| Frontend Page | `pages/register.js` | Formulário com validação |
| Store | `stores/authStore.js` | `register()`, `verifySMS()` |
| Backend Controller | `controllers/authController.js` | `register()`, `verifySMS()` |
| Service SMS | `services/sms.service.js` | `generateSMSCode()`, `sendVerificationCode()` |
| Model | `models/User.js` | 26 campos (ver PRD 2.1.1) |
| Rota | `routes/auth.js` | `POST /register`, `POST /verify-sms` |

**Campos Criados no User:**
```javascript
{
  nome: "Nome Informado",
  email: "email@exemplo.com",
  celular: "+5521999999999",
  password: "[hash bcrypt]",
  profileComplete: true,  // Já tem nome + email
  phoneVerified: false,   // → true após SMS
  smsCode: "123456",      // → null após verificação
  smsCodeExpiry: Date,    // +5 minutos
  smsAttempts: 0,         // → incrementa a cada erro
  role: "cliente",
  isActive: true
}
```

> **⚠️ DIVERGÊNCIA**: O bônus de R$10 no cadastro NÃO é automático. Deve ser adicionado manualmente via Admin/CRM.

#### 1.1.3 Cadastro Rápido (Phone-Only) ✅

```
┌─────────────────────────────────────────────────────────┐
│              CADASTRO RÁPIDO (SÓ CELULAR)               │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Tela: /register                                       │
│  ┌─────────────────────────────────────┐               │
│  │ Cadastro Rápido                     │               │
│  │                                     │               │
│  │ Celular: [+55 (21) 99999-9999]      │               │
│  │                                     │               │
│  │ [Cadastrar com Celular]             │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              ▼                                          │
│  POST /api/auth/register-phone                         │
│  • Cria user com nome temporário                       │
│  • profileComplete = false                             │
│  • Envia SMS                                           │
│              │                                          │
│              ▼                                          │
│  Tela: Verificar SMS (igual fluxo tradicional)         │
│              │                                          │
│              ▼                                          │
│  Login realizado                                       │
│              │                                          │
│              ▼                                          │
│      ┌──────────────┐                                  │
│      │profileComplete?│                                │
│      └───────┬────────┘                                │
│              │                                          │
│      ┌───────┴───────┐                                 │
│      │               │                                 │
│      ▼               ▼                                 │
│   false           true                                 │
│      │               │                                 │
│      ▼               ▼                                 │
│  /complete-      /cardapio                             │
│   profile                                              │
│      │                                                 │
│      ▼                                                 │
│  ┌─────────────┐                                       │
│  │ Nome: [___] │                                       │
│  │ Email:[___] │                                       │
│  │ Senha:[___] │                                       │
│  │  (opcional) │                                       │
│  └─────────────┘                                       │
│      │                                                 │
│      ▼                                                 │
│  profileComplete = true                                │
│      │                                                 │
│      ▼                                                 │
│  /cardapio                                             │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

#### 1.1.4 Login/Cadastro com Google OAuth

```
┌─────────────────────────────────────────────────────────┐
│               LOGIN/CADASTRO COM GOOGLE                 │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Tela: /login ou /register                             │
│  ┌─────────────────────────────────────┐               │
│  │                                     │               │
│  │   [🔵 Entrar com Google]            │               │
│  │                                     │               │
│  │   ─────── ou ───────                │               │
│  │                                     │               │
│  │   Email/Celular: [__________]       │               │
│  │   Senha: [__________]               │               │
│  │                                     │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Clica "Entrar com Google"               │
│              ▼                                          │
│  ┌─────────────────────────────────────┐               │
│  │  Google Login Popup                 │               │
│  │                                     │               │
│  │  • Escolhe conta Google             │               │
│  │  • Autoriza FLAME                   │               │
│  │                                     │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              ▼                                          │
│  Callback: credential JWT recebido                     │
│              │                                          │
│              ▼                                          │
│  POST /api/auth/google { credential }                  │
│              │                                          │
│              ▼                                          │
│  Backend valida token com Google API                   │
│              │                                          │
│              ▼                                          │
│      ┌──────────────┐                                  │
│      │Usuário existe?│                                 │
│      └───────┬───────┘                                 │
│              │                                          │
│      ┌───────┴───────┐                                 │
│      │               │                                 │
│      ▼               ▼                                 │
│    SIM             NÃO                                 │
│      │               │                                 │
│      │               ▼                                 │
│      │    Cria novo usuário:                           │
│      │    • googleId = sub                             │
│      │    • email (do Google)                          │
│      │    • nome (do Google)                           │
│      │    • profileComplete = true                     │
│      │    • phoneVerified = false                      │
│      │               │                                 │
│      │  Toast: "Bem-vindo ao FLAME! 🔥"                │
│      │               │                                 │
│      └───────┬───────┘                                 │
│              │                                          │
│              ▼                                          │
│  JWT gerado + user retornado                           │
│              │                                          │
│              ▼                                          │
│  Zustand: salva { user, token }                        │
│              │                                          │
│              ▼                                          │
│  /cardapio (acesso imediato)                           │
│                                                         │
│  Nota: Celular pode ser adicionado                     │
│        posteriormente em /perfil                       │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

#### 1.1.5 Login SMS

```
Tela: /login
    │
    ├── Input: Celular
    │
    ▼
[Enviar código] → SMS enviado
    │
    ▼
Input: Código 6 dígitos
    │
    ▼
[Verificar] → Válido?
    │
    ├── Sim → JWT gerado → Sessão criada → /cardapio
    │
    └── Não → Erro "Código inválido"
```

---

### 1.2 Pedido na Mesa

#### 1.2.1 Fluxo Completo de Pedido

```
┌─────────────────────────────────────────────────────────┐
│               FLUXO DE PEDIDO (MESA)                    │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  CARDÁPIO (/cardapio)                                  │
│  ┌─────────────────────────────────────┐               │
│  │ [🔍 Buscar...]                      │               │
│  │                                     │               │
│  │ Categorias:                         │               │
│  │ [Drinks] [Cervejas] [Comidas] [Nar- │               │
│  │                              guilé] │               │
│  │                                     │               │
│  │ ┌─────────┐ ┌─────────┐            │               │
│  │ │ Produto │ │ Produto │ ...        │               │
│  │ │ R$ XX   │ │ R$ XX   │            │               │
│  │ │ [+]     │ │ [+]     │            │               │
│  │ └─────────┘ └─────────┘            │               │
│  │                                     │               │
│  │ 🛒 Carrinho (3) - R$ 89,00 [Ver]   │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Clica em produto                        │
│              ▼                                          │
│  MODAL PRODUTO                                         │
│  ┌─────────────────────────────────────┐               │
│  │ [Imagem do Produto]                 │               │
│  │                                     │               │
│  │ Nome do Produto                     │               │
│  │ R$ 35,00                            │               │
│  │                                     │               │
│  │ Descrição do produto lorem ipsum    │               │
│  │ dolor sit amet...                   │               │
│  │                                     │               │
│  │ Quantidade: [-] 1 [+]               │               │
│  │                                     │               │
│  │ Observações:                        │               │
│  │ [Sem gelo, por favor]               │               │
│  │                                     │               │
│  │ [Adicionar ao Carrinho - R$ 35,00]  │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Adiciona                                │
│              ▼                                          │
│  Toast: "Adicionado! 🔥"                               │
│              │                                          │
│              │ Continua navegando ou...                │
│              ▼                                          │
│  CARRINHO (/carrinho)                                  │
│  ┌─────────────────────────────────────┐               │
│  │ Seus Itens                          │               │
│  │                                     │               │
│  │ ┌───────────────────────────────┐   │               │
│  │ │ Caipirinha    [-] 2 [+]  R$50 │   │               │
│  │ │ [🗑️]                          │   │               │
│  │ └───────────────────────────────┘   │               │
│  │ ┌───────────────────────────────┐   │               │
│  │ │ Porção Fritas [-] 1 [+]  R$35 │   │               │
│  │ │ [🗑️]                          │   │               │
│  │ └───────────────────────────────┘   │               │
│  │                                     │               │
│  │ ─────────────────────────────────   │               │
│  │ Subtotal:              R$ 85,00     │               │
│  │ Taxa de serviço (10%): R$ 8,50 [x]  │ ← Removível  │
│  │ TOTAL:                 R$ 93,50     │               │
│  │                                     │               │
│  │ [Continuar Comprando]               │               │
│  │ [Finalizar Pedido]                  │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Finalizar                               │
│              ▼                                          │
│  CHECKOUT (/checkout) ⚠️ ATUALIZADO                    │
│  ┌─────────────────────────────────────┐               │
│  │ Confirme seu pedido                 │               │
│  │                                     │               │
│  │ 📍 Mesa: 07                         │               │
│  │    [Trocar mesa]                    │               │
│  │                                     │               │
│  │ 💳 PAGAR PELO APP:                  │               │
│  │    ○ Cartão de Crédito              │               │
│  │    ○ Cartão de Débito               │               │
│  │    ● PIX (Recomendado)              │               │
│  │                                     │               │
│  │ 🙋 PAGAR COM ATENDENTE:             │               │
│  │    ○ Dinheiro                       │               │
│  │    ○ Cartão na Mesa                 │               │
│  │    ○ Dividir Conta                  │               │
│  │                                     │               │
│  │ ─────────────────────────────────   │               │
│  │ Subtotal:         R$ 85,00          │               │
│  │ Taxa serviço:     R$ 8,50    [x]    │ ← Removível  │
│  │ TOTAL:            R$ 93,50          │               │
│  │                                     │               │
│  │ ─────────────────────────────────   │               │
│  │ 🔥 Ganhe 5% de cashback!            │               │
│  │ Poste no Instagram @flamelounge_    │               │
│  │ Seu @: [@___________]               │               │
│  │ [✓] Quero participar                │               │
│  │                                     │               │
│  │ [Confirmar Pedido]                  │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Confirma                                │
│              ▼                                          │
│  PAGAMENTO (se PIX)                                    │
│  ┌─────────────────────────────────────┐               │
│  │ [QR Code PIX]                       │               │
│  │                                     │               │
│  │ Valor: R$ 93,50                     │               │
│  │                                     │               │
│  │ [Copiar código]                     │               │
│  │                                     │               │
│  │ ⏱️ Expira em: 14:58                 │               │
│  │                                     │               │
│  │ Aguardando pagamento...             │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Pagamento confirmado                    │
│              ▼                                          │
│  PEDIDO CRIADO                                         │
│  ┌─────────────────────────────────────┐               │
│  │ ✅ Pedido #0127 confirmado!         │               │
│  │                                     │               │
│  │ Acompanhe em tempo real:            │               │
│  │                                     │               │
│  │ [Ver meu pedido]                    │               │
│  └─────────────────────────────────────┘               │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

#### 1.2.1b Fluxo Pagamento com Atendente ✅ IMPLEMENTADO (Sprint A)

```
┌─────────────────────────────────────────────────────────────────────┐
│               PAGAMENTO COM ATENDENTE + TROCO                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  CHECKOUT - Cliente seleciona forma de pagamento:                  │
│              │                                                      │
│              ▼                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ 🙋 PAGAR COM ATENDENTE:                                     │    │
│  │    ○ Dinheiro                                               │    │
│  │    ○ Cartão na Mesa                                         │    │
│  │    ○ Dividir Conta                                          │    │
│  │    ● Pagar Depois                                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              │ Se selecionar DINHEIRO →                             │
│              ▼                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ 💰 Precisa de troco?       [Toggle ON]                      │    │
│  │                                                             │    │
│  │ Troco para quanto?                                          │    │
│  │ [R$ 100,00_]                                                │    │
│  │                                                             │    │
│  │ ✅ Troco: R$ 6,50                                           │    │
│  │ (Total: R$ 93,50)                                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              ▼                                                      │
│  PEDIDO CRIADO (status: pending_payment)                            │
│  • paymentMethod: 'cash'                                            │
│  • notes inclui: "[TROCO] Cliente precisa de troco para R$ 100,00  │
│                   (Troco: R$ 6,50)"                                 │
│              │                                                      │
│              ▼                                                      │
│  NOTIFICAÇÃO SOCKET.IO → Atendentes (room: 'attendants')            │
│  Event: 'payment_request'                                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ 🔔 PAGAMENTO NA MESA                                        │    │
│  │                                                             │    │
│  │ Mesa 07 │ Pedido #0127 │ R$ 93,50                           │    │
│  │ Forma: DINHEIRO                                             │    │
│  │ ⚠️ Cliente precisa de troco para R$ 100,00                  │    │
│  │                                                             │    │
│  │ [Ver Pedido]                                                │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              ▼                                                      │
│  Painel Atendente - ABA "PAGAMENTOS (1)":                           │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Mesa 07 │ Pedido #0127                                      │    │
│  │                                                             │    │
│  │ Cliente: João Silva                                         │    │
│  │ Itens: 2x Caipirinha, 1x Porção Fritas                      │    │
│  │                                                             │    │
│  │ Total: R$ 93,50                                             │    │
│  │ Forma: Dinheiro                                             │    │
│  │ Esperando: 2 min                                            │    │
│  │                                                             │    │
│  │ [Confirmar Pagamento]                                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              │ Clica "Confirmar Pagamento"                          │
│              ▼                                                      │
│  MODAL CONFIRMAÇÃO:                                                 │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ 💳 Confirmar Pagamento                                      │    │
│  │                                                             │    │
│  │ Pedido: #0127 │ Mesa: 07                                    │    │
│  │ Cliente: João Silva                                         │    │
│  │ Forma: Dinheiro                                             │    │
│  │                                                             │    │
│  │ Total a Receber: R$ 93,50                                   │    │
│  │                                                             │    │
│  │ Valor Recebido (opcional): R$ [100,00]                      │    │
│  │                                                             │    │
│  │ 💡 Troco: R$ 6,50                                           │    │
│  │                                                             │    │
│  │ [Confirmar Recebimento] [Cancelar]                          │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              │ Confirma                                             │
│              ▼                                                      │
│  POST /api/orders/:id/confirm-payment                               │
│  { amountReceived: 100.00, change: 6.50 }                           │
│              │                                                      │
│              ▼                                                      │
│  Backend atualiza:                                                  │
│  • order.status = 'confirmed'                                       │
│  • order.paymentStatus = 'completed'                                │
│  • order.attendantId = atendente.id                                 │
│  • order.confirmedAt = new Date()                                   │
│              │                                                      │
│              ▼                                                      │
│  Se cash → Registra em CashMovement:                                │
│  • type: 'entrada'                                                  │
│  • amount: 93.50                                                    │
│  • amountReceived: 100.00                                           │
│  • change: 6.50                                                     │
│              │                                                      │
│              ▼                                                      │
│  Socket.IO notifica:                                                │
│  • Cliente: "payment_confirmed" → "Pagamento confirmado! Seu        │    │
│    pedido está sendo preparado."                                    │
│  • Cozinha/Bar: "new_kitchen_order"/"new_bar_order" → Mostra        │    │
│    pedido para preparo                                              │
│  • Caixa: Atualiza dashboard                                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Arquivos Implementados:**
- ✅ Backend: [orderController.js](d:\flame\backend\src\controllers\orderController.js) - `createOrder()`, `confirmAttendantPayment()`, `getPendingPayments()`
- ✅ Backend: [Order.js](d:\flame\backend\src\models\Order.js) - `isAttendantPayment()`, status `pending_payment`
- ✅ Backend: [socket.service.js](d:\flame\backend\src\services\socket.service.js) - `notifyPaymentRequest()`, `notifyPaymentConfirmed()`
- ✅ Backend: [routes/orders.js](d:\flame\backend\src\routes\orders.js) - Rotas `/pending-payments`, `/:id/confirm-payment`
- ✅ Frontend: [checkout.js](d:\flame\frontend\src\pages\checkout.js) - UI de troco no Step 3
- ✅ Frontend: [atendente/index.js](d:\flame\frontend\src\pages\atendente\index.js) - Aba "Pagamentos" + Modal de confirmação

#### 1.2.1c Fluxo Divisão de Conta (NOVO!)

```
┌─────────────────────────────────────────────────────────────────────┐
│                      DIVISÃO DE CONTA                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Cliente seleciona "Dividir Conta"                                  │
│              │                                                      │
│              ▼                                                      │
│  Atendente é notificado → Vai à mesa                                │
│              │                                                      │
│              ▼                                                      │
│  Painel Atendente:                                                  │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Mesa 07 │ DIVIDIR CONTA                                     │    │
│  │                                                             │    │
│  │ Total: R$ 220,00 (2 pedidos)                                │    │
│  │                                                             │    │
│  │ Dividir em:                                                 │    │
│  │   ● Partes iguais: [2] [3] [4] [5] [6]                     │    │
│  │     R$ 110,00 por pessoa (2 pessoas)                        │    │
│  │                                                             │    │
│  │   ○ Valores diferentes                                      │    │
│  │                                                             │    │
│  │ ─────────────────────────────────────────────────────────   │    │
│  │                                                             │    │
│  │ Pessoa 1:                                                   │    │
│  │   Valor: R$ 110,00                                          │    │
│  │   Forma: [Crédito ▼]  [Pago ✓]                             │    │
│  │                                                             │    │
│  │ Pessoa 2:                                                   │    │
│  │   Valor: R$ 110,00                                          │    │
│  │   Forma: [PIX ▼]      [Aguardando]                         │    │
│  │                                                             │    │
│  │ ─────────────────────────────────────────────────────────   │    │
│  │ Restante: R$ 0,00                                           │    │
│  │                                                             │    │
│  │ [Finalizar Divisão]                                         │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│              ▼                                                      │
│  Quando todos pagaram:                                              │
│  - Pedido(s) confirmado(s)                                          │
│  - Registra movimento no caixa                                      │
│  - Cada cliente recebe notificação                                  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

#### 1.2.1d Fluxo Cashback Instagram (NOVO!)

```
┌─────────────────────────────────────────────────────────────────────┐
│                    CASHBACK INSTAGRAM                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  No checkout, cliente marca "Quero participar"                      │
│  Informa @instagram                                                 │
│              │                                                      │
│              ▼                                                      │
│  Pedido criado com: instagramPromoOptIn = true                      │
│              │                                                      │
│              ▼                                                      │
│  Pedido preparado → Atendente vai entregar                          │
│              │                                                      │
│              ▼                                                      │
│  Painel Atendente (na entrega):                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │ Pedido #0127 │ Mesa 07                                      │    │
│  │                                                             │    │
│  │ 📸 CLIENTE PARTICIPA DO INSTAGRAM CASHBACK                  │    │
│  │                                                             │    │
│  │ Instagram do cliente: @usuario_cliente                      │    │
│  │                                                             │    │
│  │ ─────────────────────────────────────────────────────────   │    │
│  │                                                             │    │
│  │ ℹ️ Instruções:                                              │    │
│  │ 1. Peça para o cliente mostrar o celular                   │    │
│  │ 2. Verifique se postou foto do pedido                      │    │
│  │ 3. Verifique se marcou @flamelounge_                       │    │
│  │                                                             │    │
│  │ ─────────────────────────────────────────────────────────   │    │
│  │                                                             │    │
│  │ [✓ Cliente Postou] [✗ Não Postou]                          │    │
│  └─────────────────────────────────────────────────────────────┘    │
│              │                                                      │
│      ┌───────┴───────┐                                             │
│      │               │                                             │
│      ▼               ▼                                             │
│  Confirmou       Não postou                                        │
│      │               │                                             │
│      ▼               ▼                                             │
│  Credita 5%    Pedido normal                                       │
│  cashback      (sem cashback                                       │
│  extra          Instagram)                                         │
│      │                                                             │
│      ▼                                                             │
│  Notificação para cliente:                                         │
│  "🎉 Você ganhou R$ X,XX de cashback                               │
│   pela sua postagem no Instagram!"                                 │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

#### 1.2.2 Acompanhamento do Pedido

```
┌─────────────────────────────────────────────────────────┐
│              TRACKING (/pedido/{id})                    │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Pedido #0127                                          │
│  Mesa 07                                               │
│                                                         │
│  Timeline:                                             │
│                                                         │
│  ✅ Pedido recebido         14:32                      │
│  │                                                     │
│  ✅ Em preparo              14:35                      │
│  │  └─ Cozinha: Porção Fritas                         │
│  │  └─ Bar: Caipirinha (2)                            │
│  │                                                     │
│  🔄 Pronto para entrega     --:--                      │
│  │                                                     │
│  ○ Entregue                 --:--                      │
│                                                         │
│  ─────────────────────────────────────                 │
│                                                         │
│  Itens:                                                │
│  • 2x Caipirinha            R$ 50,00                   │
│  • 1x Porção Fritas         R$ 35,00                   │
│                                                         │
│  Total pago: R$ 83,50                                  │
│  (+R$ 4,18 de cashback acumulado!)                     │
│                                                         │
│  [Preciso de ajuda]                                    │
│                                                         │
└─────────────────────────────────────────────────────────┘

Eventos Socket.IO:
- 'order_status_update' → Atualiza timeline
- 'order_ready' → Notificação push
- 'order_delivered' → Mostra botão avaliar
```

---

### 1.3 Pedido no Balcão

```
┌─────────────────────────────────────────────────────────┐
│               FLUXO BALCÃO                              │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Acesso direto (sem QR)                                │
│  URL: flame.com.br                                     │
│              │                                          │
│              ▼                                          │
│  Login/Cadastro (igual)                                │
│              │                                          │
│              ▼                                          │
│  Cardápio (sem mesa definida)                          │
│              │                                          │
│              ▼                                          │
│  Carrinho → Checkout                                   │
│              │                                          │
│              ▼                                          │
│  SELEÇÃO DE ENTREGA                                    │
│  ┌─────────────────────────────────────┐               │
│  │ Como deseja receber?                │               │
│  │                                     │               │
│  │ ○ Estou em uma mesa                 │               │
│  │   [Selecionar mesa ▼]               │               │
│  │                                     │               │
│  │ ● Retirar no balcão                 │               │
│  │   Você receberá uma notificação     │               │
│  │   quando estiver pronto!            │               │
│  │                                     │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Marca "Balcão"                          │
│              ▼                                          │
│  Pagamento → Pedido criado                             │
│              │                                          │
│              ▼                                          │
│  Tracking especial:                                    │
│  ┌─────────────────────────────────────┐               │
│  │ Pedido #0128 (BALCÃO)               │               │
│  │                                     │               │
│  │ ✅ Recebido                         │               │
│  │ 🔄 Preparando...                    │               │
│  │ ○ Pronto - Retire no balcão         │               │
│  │                                     │               │
│  │ Quando estiver pronto, você         │               │
│  │ receberá uma notificação.           │               │
│  │ Procure nosso atendente.            │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Status = 'ready'                        │
│              ▼                                          │
│  NOTIFICAÇÃO PUSH:                                     │
│  "🔥 Pedido #0128 pronto! Retire no balcão"           │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

### 1.4 Narguilé

```
┌─────────────────────────────────────────────────────────┐
│               FLUXO NARGUILÉ                            │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  No Cardápio: Categoria "Narguilé"                     │
│              │                                          │
│              ▼                                          │
│  SELEÇÃO DE SABOR                                      │
│  ┌─────────────────────────────────────┐               │
│  │ 🌀 Narguilé FLAME                   │               │
│  │                                     │               │
│  │ R$ 60/hora (sabor incluso)          │               │
│  │ Mínimo 30 minutos                   │               │
│  │                                     │               │
│  │ Escolha o sabor:                    │               │
│  │                                     │               │
│  │ CLÁSSICOS (inclusos):               │               │
│  │ ○ Menta                             │               │
│  │ ○ Limão                             │               │
│  │ ● Uva                               │               │
│  │ ○ Maçã                              │               │
│  │                                     │               │
│  │ PREMIUM (+R$15):                    │               │
│  │ ○ Blue Ice                          │               │
│  │ ○ Love 66                           │               │
│  │ ○ Mint Chocolate                    │               │
│  │                                     │               │
│  │ SIGNATURE (+R$20):                  │               │
│  │ ○ FLAME Special (segredo da casa)   │               │
│  │                                     │               │
│  │ [Adicionar ao Pedido]               │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Adiciona                                │
│              ▼                                          │
│  No carrinho aparece:                                  │
│  "Narguilé - Uva (tempo a definir)"                   │
│              │                                          │
│              │ Faz checkout                            │
│              ▼                                          │
│  Pedido criado → Bar recebe                            │
│              │                                          │
│              │ Bar prepara e entrega                   │
│              ▼                                          │
│  TIMER INICIA (no sistema)                             │
│  ┌─────────────────────────────────────┐               │
│  │ 🌀 Seu Narguilé                     │               │
│  │                                     │               │
│  │ Sabor: Uva                          │               │
│  │ Mesa: 07                            │               │
│  │                                     │               │
│  │     ⏱️ 00:42:15                     │               │
│  │                                     │               │
│  │ Próxima troca de carvão: 02:45      │               │
│  │                                     │               │
│  │ [Solicitar troca de carvão]         │               │
│  │ [Encerrar narguilé]                 │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Cliente encerra                         │
│              ▼                                          │
│  FECHAMENTO                                            │
│  ┌─────────────────────────────────────┐               │
│  │ Narguilé encerrado                  │               │
│  │                                     │               │
│  │ Tempo total: 1h 15min               │               │
│  │ Valor: R$ 75,00                     │               │
│  │ (R$60 primeira hora + R$15 15min)   │               │
│  │                                     │               │
│  │ Adicionado à sua conta!             │               │
│  └─────────────────────────────────────┘               │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

### 1.5 Reserva de Mesa

```
┌─────────────────────────────────────────────────────────┐
│               FLUXO RESERVA                             │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Menu → "Reservar Mesa"                                │
│              │                                          │
│              ▼                                          │
│  CALENDÁRIO                                            │
│  ┌─────────────────────────────────────┐               │
│  │ Reservar Mesa                       │               │
│  │                                     │               │
│  │       Dezembro 2024                 │               │
│  │ Dom Seg Ter Qua Qui Sex Sáb        │               │
│  │                           1   2     │               │
│  │   3   4   5   6   7  [8]  9        │               │
│  │  ...                                │               │
│  │                                     │               │
│  │ Horários disponíveis (08/12):       │               │
│  │                                     │               │
│  │ [18:00] [19:00] [20:00] [21:00]    │               │
│  │ [22:00]                             │               │
│  │                                     │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Seleciona 20:00                         │
│              ▼                                          │
│  DETALHES                                              │
│  ┌─────────────────────────────────────┐               │
│  │ Quantas pessoas?                    │               │
│  │                                     │               │
│  │      [-]   4   [+]                  │               │
│  │                                     │               │
│  │ Observações:                        │               │
│  │ [Aniversário, mesa perto do bar]    │               │
│  │                                     │               │
│  │ ─────────────────────────────────   │               │
│  │ 📅 08/12 às 20:00                   │               │
│  │ 👥 4 pessoas                        │               │
│  │                                     │               │
│  │ [Solicitar Reserva]                 │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              ▼                                          │
│  CONFIRMAÇÃO                                           │
│  ┌─────────────────────────────────────┐               │
│  │ ✅ Reserva solicitada!              │               │
│  │                                     │               │
│  │ Você receberá uma confirmação       │               │
│  │ em breve via SMS.                   │               │
│  │                                     │               │
│  │ 📅 08/12 às 20:00                   │               │
│  │ 👥 4 pessoas                        │               │
│  │ Status: Aguardando confirmação      │               │
│  │                                     │               │
│  │ (Sistema envia WhatsApp para FLAME  │               │
│  │  com os detalhes da reserva)        │               │
│  │                                     │               │
│  │ [Ver minhas reservas]               │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Staff confirma                          │
│              ▼                                          │
│  SMS: "🔥 Reserva confirmada! 08/12 às 20:00"         │
│                                                         │
│  No dia, 2h antes:                                     │
│  WhatsApp: "Lembrete: sua reserva é hoje às 20:00!"   │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

### 1.6 Programa de Cashback

```
┌─────────────────────────────────────────────────────────┐
│               FLUXO CASHBACK                            │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Menu → "Meu Cashback"                                 │
│              │                                          │
│              ▼                                          │
│  DASHBOARD CASHBACK                                    │
│  ┌─────────────────────────────────────┐               │
│  │ 🔥 FLAME Rewards                    │               │
│  │                                     │               │
│  │       R$ 34,00                      │               │
│  │    cashback disponível              │               │
│  │                                     │               │
│  │ Nível: SILVER 🥈                    │               │
│  │ Cashback: 5% em cada compra         │               │
│  │                                     │               │
│  │ ████████░░░░░░░ R$160 p/ Gold      │               │
│  │ Você já gastou R$4.840 (total)      │               │
│  │                                     │               │
│  │ [Ver Benefícios]  [Histórico]       │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Ver benefícios                          │
│              ▼                                          │
│  BENEFÍCIOS POR TIER                                   │
│  ┌─────────────────────────────────────┐               │
│  │ Seu nível: SILVER 🥈                │               │
│  │                                     │               │
│  │ ✅ 5% de cashback                   │               │
│  │ ✅ Prioridade em reservas           │               │
│  │ ✅ R$ 50 no aniversário             │               │
│  │                                     │               │
│  │ Próximo nível: GOLD 🥇              │               │
│  │ Faltam R$ 160 em compras            │               │
│  │                                     │               │
│  │ Benefícios do Gold:                 │               │
│  │ • 8% de cashback                    │               │
│  │ • Mesa reservada                    │               │
│  │ • R$ 100 no aniversário             │               │
│  │ • 1 drink cortesia/mês              │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              │ Ver histórico                           │
│              ▼                                          │
│  HISTÓRICO CASHBACK                                    │
│  ┌─────────────────────────────────────┐               │
│  │ Últimas transações                  │               │
│  │                                     │               │
│  │ 05/12 │ Pedido #127  │ +R$ 4,18    │               │
│  │ 03/12 │ Usado        │ -R$ 10,00   │               │
│  │ 01/12 │ Pedido #119  │ +R$ 3,50    │               │
│  │ 28/11 │ Pedido #112  │ +R$ 5,25    │               │
│  │                                     │               │
│  │ Saldo atual: R$ 34,00               │               │
│  └─────────────────────────────────────┘               │
│                                                         │
└─────────────────────────────────────────────────────────┘

**Como funciona hoje:**
|- Você ganha cashback automaticamente em cada pedido entregue e pago, de acordo com seu tier.
|- O saldo e o histórico ficam visíveis no módulo "Meu Cashback" (app) e nas telas de CRM/Admin.
|- O uso como desconto direto no checkout ainda **não está ativo**; a regra planejada é permitir usar até cerca de 50% do valor do pedido em cashback.

**Validade:**
|- Um job diário expira saldos de cashback que ficaram mais de 90 dias sem novas transações de ganho ou bônus.
```

---

## 2. FLUXOS DO STAFF

### 2.1 Login Staff

```
┌─────────────────────────────────────────────────────────┐
│               LOGIN FUNCIONÁRIO                         │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  URL: /staff/login                                     │
│  ┌─────────────────────────────────────┐               │
│  │ 🔥 FLAME Staff                      │               │
│  │                                     │               │
│  │ Email:                              │               │
│  │ [joao@flame.com.br]                 │               │
│  │                                     │               │
│  │ Senha:                              │               │
│  │ [••••••••]                          │               │
│  │                                     │               │
│  │ [Entrar]                            │               │
│  └─────────────────────────────────────┘               │
│              │                                          │
│              ▼                                          │
│  Sistema verifica role → Redireciona                   │
│                                                         │
│  role = 'kitchen'   → /staff/cozinha                   │
│  role = 'bar'       → /staff/bar                       │
│  role = 'attendant' → /staff/atendente                 │
│  role = 'cashier'   → /staff/caixa                     │
│  role = 'manager'   → /staff/dashboard                 │
│  role = 'admin'     → /admin                           │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

### 2.2 Painel Cozinha

```
┌─────────────────────────────────────────────────────────┐
│               COZINHA (/staff/cozinha)                  │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │ 🍳 Cozinha         [🔔 3]        João 👤       │   │
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│  FILA DE PEDIDOS (comida)                              │
│                                                         │
│  ┌─────────────────────────────────────┐               │
│  │ #0127 │ Mesa 07 │ 🕐 12min         │  NOVO        │
│  │ ─────────────────────────────────   │               │
│  │ • 1x Porção Fritas                  │               │
│  │ • 1x Hambúrguer Artesanal           │               │
│  │   (Obs: Sem cebola)                 │               │
│  │                                     │               │
│  │ [Iniciar Preparo]                   │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  ┌─────────────────────────────────────┐               │
│  │ #0125 │ Mesa 03 │ 🕐 18min         │  PREPARANDO  │
│  │ ─────────────────────────────────   │  ⚠️ ATRASANDO│
│  │ • 2x Porção Bolinho                 │               │
│  │                                     │               │
│  │ [Marcar Pronto]                     │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  ┌─────────────────────────────────────┐               │
│  │ #0124 │ Balcão │ 🕐 05min          │  PREPARANDO  │
│  │ ─────────────────────────────────   │               │
│  │ • 1x Tábua de Frios                 │               │
│  │                                     │               │
│  │ [Marcar Pronto]                     │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  ───────────────────────────────────────               │
│  📊 Hoje: 47 pedidos │ Tempo médio: 14min             │
│                                                         │
└─────────────────────────────────────────────────────────┘

Eventos Socket.IO (recebe):
- 'new_kitchen_order' → Adiciona card + som

Eventos Socket.IO (emite):
- 'start_preparing' → {orderId, itemIds}
- 'item_ready' → {orderId, itemId}
- 'order_kitchen_ready' → {orderId}
```

### 2.3 Painel Bar

> **NOTA**: O Bar agora gerencia APENAS bebidas. Narguilé foi migrado para o Atendente.

```
┌─────────────────────────────────────────────────────────┐
│               BAR (/staff/bar)                          │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  [BEBIDAS]                              Maria 👤       │
│                                                         │
│  === FILA DE BEBIDAS ===                               │
│                                                         │
│  EM PREPARO (2)                                        │
│  ┌─────────────────────────────────────┐               │
│  │ #0127 │ Mesa 07 │ 🕐 05min          │               │
│  │ ─────────────────────────────────   │               │
│  │ • 2x Caipirinha                     │               │
│  │ • 1x Gin Tônica                     │               │
│  │                                     │               │
│  │ [Pronto] ← Botão ativo              │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  AGUARDANDO (1)                                        │
│  ┌─────────────────────────────────────┐               │
│  │ #0129 │ Mesa 03 │ NOVO              │               │
│  │ ─────────────────────────────────   │               │
│  │ • 3x Cerveja Artesanal              │               │
│  │                                     │               │
│  │ [Preparar] ← Primeiro botão         │               │
│  └─────────────────────────────────────┘               │
│                                                         │
└─────────────────────────────────────────────────────────┘

Fluxo de Botões (Bar/Cozinha):
1. Pedido chega → Status "pending" → Botão [Preparar]
2. Clica [Preparar] → Status "preparing" → Botão [Pronto]
3. Clica [Pronto] → Status "ready" → Pedido SAI da tela
   → Vai para tela do Atendente
```

### 2.4 Painel Atendente

> **NOTA**: Atendente agora controla também o NARGUILÉ (migrado do Bar)

```
┌─────────────────────────────────────────────────────────┐
│              ATENDENTE (/atendente)                     │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  [PRONTOS]  [ENTREGUES]  [BALCÃO]  [NARGUILÉ]  Pedro 👤│
│                                                         │
│  === ABA PRONTOS ===                                   │
│                                                         │
│  EM ENTREGA (status: on_way)                           │
│  ┌─────────────────────────────────────┐               │
│  │ #0124 │ Mesa 07 │ 🕐 1min          │  COMIGO       │
│  │ ─────────────────────────────────   │               │
│  │ • 1x Tábua de Frios                 │               │
│  │ • 1x Cerveja                        │               │
│  │                                     │               │
│  │ [Entregar] ← Finaliza entrega       │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  PRONTOS PARA RETIRAR (status: ready)                  │
│  ┌─────────────────────────────────────┐               │
│  │ #0125 │ Mesa 03 │ 🕐 2min pronto    │               │
│  │ ─────────────────────────────────   │               │
│  │ • 2x Porção Bolinho (COZINHA ✅)    │               │
│  │ • 2x Caipirinha (BAR ✅)            │               │
│  │                                     │               │
│  │ [Retirar] ← Botão (ready→on_way)    │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  ⚠️ Botão fica BLOQUEADO enquanto status != ready      │
│                                                         │
│  === ABA NARGUILÉ (NOVO!) ===                          │
│                                                         │
│  ┌─────────────────────────────────────┐               │
│  │ Mesa 07 │ Uva │ ⏱️ 00:42:15        │               │
│  │ Próximo carvão: 02:45               │               │
│  │                                     │               │
│  │ [Trocar Carvão] [Pausar] [Finalizar]│               │
│  └─────────────────────────────────────┘               │
│                                                         │
└─────────────────────────────────────────────────────────┘

Fluxo de Botões (Atendente):
1. Pedido fica "ready" na Cozinha/Bar → Aparece para Atendente
2. Botão [Retirar] fica ATIVO → Clica → Status "on_way"
3. Botão [Entregar] aparece → Clica → Status "delivered"
4. Pedido SAI da tela

IMPORTANTE: Botões do Atendente ficam BLOQUEADOS até o pedido
estar "ready". Ele não pode acelerar o preparo da Cozinha/Bar.
```

### 2.5 Painel Caixa

```
┌─────────────────────────────────────────────────────────┐
│                 CAIXA (/staff/caixa)                    │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  CAIXA #1 │ Aberto 18:02 │ Carlos 👤                  │
│                                                         │
│  ┌─────────────────────────────────────┐               │
│  │ RESUMO DO TURNO                     │               │
│  │                                     │               │
│  │ Abertura:     R$ 200,00             │               │
│  │ + Dinheiro:   R$ 1.250,00           │               │
│  │ + PIX:        R$ 3.420,00           │               │
│  │ + Cartão:     R$ 2.890,00           │               │
│  │ - Sangrias:   R$ 500,00             │               │
│  │ ─────────────────────────────────   │               │
│  │ Em caixa:     R$ 950,00             │               │
│  │ Total vendas: R$ 7.560,00           │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  [Sangria]  [Suprimento]  [Venda Manual]              │
│                                                         │
│  ÚLTIMAS MOVIMENTAÇÕES                                 │
│                                                         │
│  18:45 │ Venda #0128 │ PIX     │ +R$ 93,50            │
│  18:42 │ Venda #0127 │ Cartão  │ +R$ 156,00           │
│  18:30 │ Sangria     │ -       │ -R$ 200,00           │
│  18:15 │ Venda #0125 │ Dinheiro│ +R$ 85,00            │
│  ...                                                   │
│                                                         │
│  [Fechar Caixa]                                        │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

## 3. FLUXOS ADMINISTRATIVOS

### 3.1 Dashboard Gerente

```
┌─────────────────────────────────────────────────────────┐
│             DASHBOARD (/admin/dashboard)                │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  🔥 FLAME Dashboard          Hoje: 05/12     Admin 👤  │
│                                                         │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐      │
│  │ R$7.5k  │ │   67    │ │ R$112   │ │  12     │      │
│  │ Vendas  │ │ Pedidos │ │ Ticket  │ │ Mesas   │      │
│  │ +15% ↑  │ │ +8% ↑   │ │ +3% ↑   │ │ ativas  │      │
│  └─────────┘ └─────────┘ └─────────┘ └─────────┘      │
│                                                         │
│  VENDAS POR HORA                                       │
│  ┌─────────────────────────────────────┐               │
│  │     📊 [Gráfico de barras]          │               │
│  │                                     │               │
│  │  18h  19h  20h  21h  22h  23h      │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  PEDIDOS EM TEMPO REAL                                 │
│  ┌─────────────────────────────────────┐               │
│  │ • #0130 │ Mesa 08 │ Preparando      │               │
│  │ • #0129 │ Mesa 05 │ Pronto          │               │
│  │ • #0128 │ Balcão  │ Aguardando      │               │
│  │ • #0127 │ Mesa 07 │ Entregue ✅     │               │
│  └─────────────────────────────────────┘               │
│                                                         │
│  ALERTAS                                               │
│  ⚠️ Estoque baixo: Vodka Absolut (2 un)               │
│  ⚠️ Pedido #0125 atrasado (>20min)                    │
│  📅 3 reservas para hoje às 21h                        │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

---

## 4. FLUXOS DO SISTEMA

### 4.1 Processamento de Pedido

```
┌─────────────────────────────────────────────────────────┐
│            PIPELINE DE PEDIDO                           │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Cliente confirma pedido                               │
│              │                                          │
│              ▼                                          │
│  [1] VALIDAÇÃO                                         │
│      • Itens disponíveis?                              │
│      • Mesa válida?                                    │
│      • Pagamento OK?                                   │
│              │                                          │
│              ▼                                          │
│  [2] CRIAÇÃO                                           │
│      • Gera número do pedido                           │
│      • Status = 'pending_payment' ou 'confirmed'       │
│      • Registra no banco                               │
│              │                                          │
│              ▼                                          │
│  [3] PAGAMENTO (se necessário)                         │
│      • Processa Stripe                                 │
│      • Confirma PIX                                    │
│      • Status = 'confirmed'                            │
│              │                                          │
│              ▼                                          │
│  [4] DISTRIBUIÇÃO                                      │
│      • Separa itens por setor:                         │
│        - Comida → Cozinha                              │
│        - Drink → Bar                                   │
│        - Narguilé → Bar ⚠️ (deveria ser Atendente)    │
│      • Emite Socket 'new_order' para cada setor        │
│      • ⚠️ FALTA: notificar Atendente em novos pedidos │
│              │                                          │
│              ▼                                          │
│  [5] ESTOQUE                                           │
│      • Para cada item:                                 │
│        - Baixa direta no product.stock                 │
│        - ⚠️ Ficha técnica NÃO implementada            │
│        - Verifica alertas se stock < minStock          │
│              │                                          │
│              ▼                                          │
│  [6] CASHBACK                                          │
│      • Calcula cashback (valor × % do tier)            │
│      • Registra transação pendente                     │
│              │                                          │
│              ▼                                          │
│  [7] NOTIFICAÇÃO                                       │
│      • Push para cliente: "Pedido confirmado!"         │
│      • Som no painel do setor                          │
│              │                                          │
│              ▼                                          │
│  [8] TRACKING                                          │
│      • Inicia monitoramento de tempo                   │
│      • Alerta se > 15min                               │
│                                                         │
└─────────────────────────────────────────────────────────┘
```

### 4.2 Fluxo de Entrega

```
Setor marca "Pronto"
       │
       ▼
Socket 'item_ready' → Atendente
       │
       ▼
Todos itens prontos?
       │
   ┌───┴───┐
   │       │
  Não     Sim
   │       │
   ▼       ▼
 Aguarda  Socket 'order_ready'
          │
          ├─→ Atendente (painel)
          │
          └─→ Cliente (se balcão: push)
               │
               ▼
       Atendente "Pega pedido"
               │
               ▼
       Atendente "Entrega"
               │
               ▼
       Status = 'delivered'
               │
               ├─→ Cliente: push "Pedido entregue!"
               │
               └─→ Cashback: confirma crédito
                      │
                      ▼
               Atualiza saldo e tier cliente
```

---

## 5. MAPEAMENTO TÉCNICO

### 5.1 Arquivos por Fluxo

| Fluxo | Frontend (Pages) | Frontend (Stores) | Backend (Routes) | Backend (Controllers) | Models |
|-------|-----------------|-------------------|------------------|----------------------|--------|
| **Cadastro/Login** | `/register`, `/login`, `/complete-profile` | `authStore.js` | `auth.js` | `authController.js` | `User.js` |
| **Pedido Mesa** | `/cardapio`, `/carrinho`, `/checkout`, `/pedido/[id]` | `cartStore.js`, `orderStore.js` | `orders.js` | `orderController.js` | `Order.js`, `OrderItem.js` |
| **Narguilé** | `/cardapio` (categoria), `/staff/bar` | `cartStore.js` | `hookah.js`, `orders.js` | `hookahController.js` | `HookahSession.js` |
| **Reserva** | `/reservas` | `reservationStore.js` | `reservations.js` | `reservationController.js` | `Reservation.js` |
| **Cashback** | `/perfil`, `/cashback` | `userStore.js` | `cashback.js`, `users.js` | `cashbackController.js` | `Cashback.js`, `CashbackTransaction.js` |
| **Cozinha** | `/cozinha/index.js` | `orderStore.js`, `staffStore.js` | `orders.js` | `orderController.js` | `Order.js`, `OrderItem.js` |
| **Bar** | `/staff/bar.js` | `orderStore.js`, `staffStore.js` | `orders.js`, `hookah.js` | `orderController.js` | `Order.js`, `HookahSession.js` |
| **Atendente** | `/atendente/index.js` | `orderStore.js`, `staffStore.js` | `orders.js` | `orderController.js` | `Order.js` |
| **Caixa** | `/staff/caixa.js` | `cashStore.js`, `orderStore.js` | `cash-register.js` | `cashRegisterController.js` | `CashRegister.js`, `CashMovement.js` |
| **Admin** | `/admin/*` (12 páginas) | `adminStore.js`, `productStore.js` | `admin.js`, `products.js` | `adminController.js` | Vários |

### 5.2 Eventos Socket.IO

| Evento | Emissor | Receptor | Payload | Descrição |
|--------|---------|----------|---------|-----------|
| `new_order` | Backend | Cozinha/Bar | `{ order, items }` | Novo pedido criado |
| `new_kitchen_order` | Backend | Cozinha | `{ order, items }` | Itens de comida |
| `new_bar_order` | Backend | Bar | `{ order, items }` | Itens de bebida |
| `order_status_update` | Backend | Cliente/Staff | `{ orderId, status, timestamp }` | Mudança de status |
| `item_ready` | Staff | Backend→Atendente | `{ orderId, itemId }` | Item pronto |
| `order_ready` | Backend | Atendente/Cliente | `{ orderId }` | Pedido completo pronto |
| `order_delivered` | Atendente | Backend→Cliente | `{ orderId }` | Pedido entregue |
| `table_updated` | Backend | Admin/Atendente | `{ tableId, status }` | Status mesa mudou |
| `hookah_timer_update` | Backend | Cliente/Bar | `{ sessionId, elapsed }` | Timer narguilé |

### 5.3 Endpoints Principais

#### Autenticação
```
POST /api/auth/register        → Cadastro completo
POST /api/auth/register-phone  → Cadastro só celular
POST /api/auth/login           → Login email/senha
POST /api/auth/login-phone     → Enviar SMS
POST /api/auth/verify-phone    → Verificar código
POST /api/auth/google          → OAuth Google
GET  /api/auth/me              → Dados usuário logado
```

#### Pedidos
```
GET    /api/orders             → Lista pedidos (filtros)
POST   /api/orders             → Criar pedido
GET    /api/orders/:id         → Detalhes pedido
PATCH  /api/orders/:id/status  → Mudar status ⚠️ (sem validação de role)
GET    /api/orders/kitchen     → Pedidos da cozinha
GET    /api/orders/bar         → Pedidos do bar
```

#### Produtos
```
GET    /api/products           → Lista produtos
GET    /api/products/:id       → Detalhes produto
POST   /api/products           → Criar (admin)
PUT    /api/products/:id       → Editar (admin)
DELETE /api/products/:id       → Remover (admin)
GET    /api/products/categories → Categorias
GET    /api/products/featured  → Produtos em destaque
PATCH  /api/products/:id/stock → Atualizar estoque
GET    /api/products/stock/low → Produtos com estoque baixo
```

#### Upload de Imagens (Sprint 30)
```
POST   /api/upload/product/:id → Upload imagem para produto
POST   /api/upload/image       → Upload genérico
DELETE /api/upload/image/:fn   → Deletar imagem

Formatos: JPEG, PNG, GIF, WebP
Limite: 5MB por arquivo
Armazenamento: /uploads/products/
```

#### Fluxo de Upload de Imagem ✅

```
┌─────────────────────────────────────────────────────────────┐
│           UPLOAD DE IMAGEM (/admin/products)                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Modal de Produto:                                          │
│  ┌───────────────────────────────────────────────────────┐  │
│  │                                                       │  │
│  │    ┌─────────────────────────────────────┐           │  │
│  │    │                                     │           │  │
│  │    │   [📤 Clique para enviar imagem]   │           │  │
│  │    │   JPEG, PNG, GIF ou WebP (max 5MB) │           │  │
│  │    │                                     │           │  │
│  │    └─────────────────────────────────────┘           │  │
│  │                                                       │  │
│  │    ou cole uma URL: [_________________________]      │  │
│  │                                                       │  │
│  └───────────────────────────────────────────────────────┘  │
│                     │                                        │
│                     ▼                                        │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Validações Frontend:                                  │  │
│  │ • Tipo: image/jpeg, png, gif, webp                   │  │
│  │ • Tamanho: <= 5MB                                    │  │
│  └───────────────────────────────────────────────────────┘  │
│                     │                                        │
│                     ▼                                        │
│  POST /api/upload/product/{productId}                       │
│  Content-Type: multipart/form-data                          │
│  Body: { image: File }                                      │
│                     │                                        │
│                     ▼                                        │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Backend (multer middleware):                          │  │
│  │ 1. Valida tipo MIME                                  │  │
│  │ 2. Valida tamanho                                    │  │
│  │ 3. Gera nome único (product-{timestamp}-{random})    │  │
│  │ 4. Salva em /uploads/products/                       │  │
│  │ 5. Atualiza product.image (se productId != 'new')    │  │
│  │ 6. Deleta imagem antiga (se existir)                 │  │
│  └───────────────────────────────────────────────────────┘  │
│                     │                                        │
│                     ▼                                        │
│  Response: { success: true, imageUrl: "..." }               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### Filtros de Estoque (Sprint 30) ✅

```
┌─────────────────────────────────────────────────────────────┐
│         GESTÃO DE ESTOQUE (/admin/products)                 │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Cards de Estatísticas:                                     │
│  ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐          │
│  │Total│ │Ativo│ │Inat.│ │Em   │ │Baixo│ │Sem  │          │
│  │ 116 │ │ 110 │ │  6  │ │Est. │ │Est. │ │Est. │          │
│  │  📦 │ │  👁  │ │  ⚫ │ │ ✅  │ │  ⚠️ │ │  ❌ │          │
│  └─────┘ └─────┘ └─────┘ └─────┘ └─────┘ └─────┘          │
│                                                             │
│  Filtros:                                                   │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ 🔍 [Buscar produtos...]                               │  │
│  │                                                       │  │
│  │ Status: [Todos ▼]  Estoque: [Todos ▼]                │  │
│  │                                                       │  │
│  │ Categoria: [Todos] [Drinks] [Petiscos] [...]         │  │
│  │                                                       │  │
│  │ Filtros ativos: [Ativos ✕] [Estoque Baixo ✕]         │  │
│  │                 [Limpar todos]                        │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### Staff
```
GET  /api/staff/dashboard      → Métricas gerente
GET  /api/staff/tables         → Mapa de mesas
POST /api/staff/tables/:id/status → Atualizar mesa
```

### 5.4 Status de Pedido (Máquina de Estados)

```
                    ┌─────────────────────────────────────────┐
                    │         ESTADOS DO PEDIDO               │
                    └─────────────────────────────────────────┘

         ┌──────────────┐
         │pending_payment│ ← Pedido criado, aguarda pagamento
         └──────┬───────┘
                │ (pagamento confirmado)
                ▼
         ┌──────────────┐
         │  confirmed   │ ← Pedido pago, aguarda preparo
         └──────┬───────┘
                │ (staff inicia preparo)
                ▼
         ┌──────────────┐
         │  preparing   │ ← Em preparo (Cozinha/Bar)
         └──────┬───────┘
                │ (todos itens prontos)
                ▼
         ┌──────────────┐
         │    ready     │ ← Pronto para retirada
         └──────┬───────┘
                │ (atendente pega)
                ▼
         ┌──────────────┐
         │   on_way     │ ← A caminho da mesa
         └──────┬───────┘
                │ (atendente entrega)
                ▼
         ┌──────────────┐
         │  delivered   │ ← Entregue (cashback creditado)
         └──────────────┘

         ⚠️ DIVERGÊNCIA: Qualquer staff pode mudar para qualquer status
            (deveria ter validações por role)
```

### 5.5 Tiers de Cashback

| Tier | Gasto Acumulado | % Cashback | Campos no User |
|------|-----------------|------------|----------------|
| Bronze | R$ 0 - 499 | 2% | `cashbackTier: 'bronze'` |
| Silver | R$ 500 - 1.999 | 5% | `cashbackTier: 'silver'` |
| Gold | R$ 2.000 - 4.999 | 8% | `cashbackTier: 'gold'` |
| Platinum | R$ 5.000+ | 10% | `cashbackTier: 'platinum'` |

**Modelo Cashback:**
- `userId` - FK para User
- `balance` - Saldo disponível
- `totalEarned` - Total ganho histórico
- `totalSpent` - Total gasto histórico

**Modelo CashbackTransaction:**
- `type`: 'earn' | 'spend' | 'bonus' | 'expire' | 'adjustment'
- `amount` - Valor da transação
- `orderId` - FK opcional (quando vinculado a pedido)
- `description` - Motivo (ex: "Cashback pedido #127")

---

## 6. DIVERGÊNCIAS CONHECIDAS

| # | Fluxo | PRD/Doc | Sistema Real | Impacto | Prioridade |
|---|-------|---------|--------------|---------|------------|
| 1 | Mudança Status | Validado por role | Qualquer um muda | Alto - integridade | P0 |
| 2 | Narguilé | Atendente controla | Bar controla | Médio - UX | P1 |
| 3 | Cashback no Checkout | Usar até 50% | Não implementado | Alto - receita | P0 |
| 4 | Bônus Cadastro | Automático R$10 | Manual via CRM | Baixo | P2 |
| 5 | Bônus Aniversário | Automático | Manual | Baixo | P2 |
| 6 | No-show Reserva | Automático | Método existe, job não | Baixo | P3 |
| 7 | Ficha Técnica | Baixa por insumo | Baixa direto produto | Médio - estoque | P1 |
| 8 | Notificação Atendente | Em novos pedidos | Não implementado | Médio - operação | P1 |

---

---

## 7. PROBLEMAS CRÍTICOS IDENTIFICADOS NA AUDITORIA

### 7.1 Segurança

| Problema | Arquivo | Impacto | Prioridade |
|----------|---------|---------|------------|
| **Webhook sem autenticação** | `orderController.js` `/payment/confirm` | Qualquer um pode confirmar pagamento | 🔴 CRÍTICO |
| **CRUD produtos sem role** | `productController.js` | Qualquer usuário cria/edita/deleta | 🔴 CRÍTICO |
| **Google credentials expostas** | `.env` no repositório | Vazamento de credenciais | 🔴 CRÍTICO |
| **WhatsApp número pessoal** | `whatsapp.service.js:16` | Privacidade comprometida | 🔴 CRÍTICO |
| **VAPID keys hardcoded** | `push.service.js:11-13` | Push inseguro | 🔴 CRÍTICO |

### 7.2 Bugs de Funcionamento

| Bug | Arquivo | Descrição | Impacto |
|-----|---------|-----------|---------|
| **QR Code URL errada** | `tableController.js` (4 locais) | Gera `/table/` mas rota é `/qr/` | QR não funciona |
| **Job no-show quebrado** | `noShow.job.js:28-35` | Usa `r.time` que não existe | No-shows não marcados |
| **Caixa desincronizado** | `orderController.js` | Vendas dinheiro não registram | Caixa incorreto |
| **Socket hookah falta** | `atendente/index.js` | Não escuta eventos do narguilé | Sem tempo real |

### 7.3 Funcionalidades Faltando

| Funcionalidade | Status | Prioridade |
|----------------|--------|------------|
| Uso de cashback no checkout | ❌ Não implementado | P0 |
| Ficha técnica (receita/insumos) | ❌ Não implementado | P1 |
| Automações CRM | ❌ Não implementado | P2 |
| Push no Service Worker | ❌ Não implementado | P1 |
| Tracking de campanhas | ❌ Não implementado | P2 |

---

## 8. CONFIRMAÇÕES DA AUDITORIA

### ✅ Confirmado: Narguilé em `/atendente`

A migração do narguilé de `/staff/bar` para `/atendente` **foi concluída** na Sprint 23.

**Localização atual**: `pages/atendente/index.js`

O atendente agora controla:
- Criar sessões de narguilé
- Trocar carvão
- Pausar/retomar
- Finalizar sessão

### ✅ Confirmado: Baixa de estoque automática

Quando um pedido é criado:
1. Valida estoque disponível
2. `Product.decrement('stock', { by: quantity })`
3. `InventoryService.recordMovement(type='saida', reason='venda')`
4. Se cancelado, restaura com `devolucao`

### ✅ Confirmado: Cashback automático

Quando pedido é entregue (status = 'delivered'):
1. Calcula % baseado no tier do usuário
2. `user.addCashback(amount, orderId)`
3. Cria registro em `CashbackHistory`
4. Atualiza `cashbackBalance` do user

### ✅ Confirmado: Tiers implementados

| Tier | Gasto Mínimo | Cashback |
|------|--------------|----------|
| Bronze | R$ 0 | 2% |
| Silver | R$ 1.000 | 5% |
| Gold | R$ 5.000 | 8% |
| Platinum | R$ 10.000 | 10% |

---

*FLAME User Flows v3.3.0 - Sincronizado com auditoria completa em 07/12/2024*
