    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-t-transparent rounded-full animate-spin mx-auto mb-4" style={{ borderColor: 'var(--theme-primary)', borderTopColor: 'transparent' }}></div>
          <p className="text-white text-lg">Carregando apresentação...</p>
        </div>
      </div>
    );
  }
        ...
        {!isPresentationMode && (
          <div className="fixed top-4 right-4 z-40 bg-black/50 backdrop-blur-sm px-4 py-2 rounded-lg">
            <span className="text-white text-sm font-medium">
              <span style={{ color: 'var(--theme-primary)' }}>{currentSlide}</span>
              <span className="text-gray-400"> / {slidesData.length}</span>
            </span>
          </div>
        )}
